...
<head>
    ...
    <title>Скачать видео с YOUTUBE.COM</title>
...
<div class="account-container register">
    <div class="content clearfix">
        <form>
            <h1 style="text-align: center">СКАЧАТЬ</h1>
            ...
            if($downloads == 1) {
                ...
                for ($i = 0; $i < count($avail_formats); $i++) {
                    ...
                    <tr>
                        ...
                        <a class="btn btn-small btn-success" data-toggle="tooltip" title="Скачать видео" href="download.php?mime=' . $avail_formats[$i]['type'] .'&title='. urlencode($my_title) .'&token=' . base64_encode($avail_formats[$i]['url']) . '"><i class="btn-icon-only icon-film"></i></a>';
                    ...
                    if($config['cloud_convert_api_key'] != 'INSERT YOUR API KEY HERE' && !empty($config['cloud_convert_api_key']) && $avail_formats[$i]['type'] != 'video/3gpp' && $file_size > 0 && $file_size) {
                        echo ' <a class="btn btn-danger btn-small" data-toggle="tooltip" title="Скачать MP3" href="download.php?type=mp3&mime=' . $avail_formats[$i]['type'] .'&title='. urlencode($my_title) .'&token=' . base64_encode($avail_formats[$i]['url']) . '"><i class="btn-icon-only icon-music"> </i></a>';
                    }
                    echo '</td></tr>';
                }
                echo '</ul>';
